...
<html lang="en">
    ...
    <div id="wrapper">
      ...
      <nav class="navbar navbar-inverse" id="forms-subnav" style="margin-left: 50px;">
        <ul class="nav navbar-nav navbar-left navbar-user">
          <li class="<?php echo (isset($submenu['active']) && $submenu['active'] == 'design')? 'active' : '' ?>"><a href="/design_request/index">All</a></li>
          <li class="<?php echo (isset($submenu['active']) && $submenu['active'] == 'design')? 'active' : '' ?>"><a href="/design_request/index/2">Approved</a></li>
          <li class="<?php echo (isset($submenu['active']) && $submenu['active'] == 'design')? 'active' : '' ?>"><a href="/design_request/index/5">Finalized</a></li>
          <li class="<?php echo (isset($submenu['active']) && $submenu['active'] == 'design')? 'active' : '' ?>"><a href="/design_request/index/12">Waiting Approval</a></li>
          <li class="<?php echo (isset($submenu['active']) && $submenu['active'] == 'design')? 'active' : '' ?>"><a href="/design_request/index/3">Rejected</a></li>
        </ul>
      </nav>
      <div id="page-wrapper">
        <a href="<?php echo base_url(); ?>design_request/submit/" class="btn btn-info">New Design Request</a>
        <div class="pager tablesorter-pager" style="display: block;">
          <a class="form-actions btn btn-success non-printable" href="/design" style="float:right;" > Back </a>
          <?php if($state == 12){?>
            <fieldset class="non-printable">
              <?php $this->load->view('waiting_signature_menu'); ?>
            </fieldset>
          <?php } ?>
        ...
        <table class="table table-bordered table-hover table-striped tablesorter">
          <thead>
            <tr>
              ...
              <?php if ($is_admin): ?>
                <th class="header">Admin<i class="fa fa-sort"></i></th>
              <?php endif; ?>
          ...
          <tbody>
            <?php foreach($designs as $design ){?>
              <tr class="active">
                ...
                <?php endif; ?>
                <td><?php echo $design['hotel_name'] ?></td>
                <td><?php echo $design['date'] ?></td>
                <td><?php echo $design['name'] ?></td>
                <td><?php echo $design['department'] ?></td>
                <td><?php echo $design['timestamp'] ?></td>
                <td>
                  <a href="<?php echo base_url(); ?>design_request/view/<?php echo $design['serial'] ?> " class="btn btn-primary" style="width: 80px;">View</a>
                </td>
                <td style="width: 200px;">
                  <?php foreach ($design['approvals'] as $approval): ?>
  ...
  <script type="text/javascript">
    $(function(){
      ...
      .find(".tablesorter-filter-row td:last input").replaceWith('<a href="<?php echo base_url(); ?>design_request/" class="reset-filters btn btn-warning">Reset</a>');
    });
  </script>
  <script type="text/javascript">
    document.body.addEventListener("keydown", function (event) {
      if (event.keyCode === 13) {
        window.location.replace("<?= base_url(); ?>design_request/submit");
      }
    });  
  </script>
</html>
