...
class late_ch extends CI_Controller {
  private $data;
    ...
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_sky']) && $this->data['is_sky']) || (isset($this->data['is_UK']) && $this->data['is_UK'])) {
      ...
      if ($this->input->post('submit')) {
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          $ch_id = $this->late_ch_model->create_ch($form_data);
          if ($ch_id) {
              $this->load->model('late_ch_model');
              $this->late_ch_model->update_files($assumed_id,$ch_id);
            } else {
          ...
          foreach ($this->input->post('items') as $item) {
            $item['ch_id'] = $ch_id;  
            $item_id = $this->late_ch_model->create_room($item);   
            $resulte[] = $item['id'];
            if (!$item_id) {
              die("ERROR");
            }
          }
          $signatures = $this->late_ch_model->ch_sign($form_data['type']);
          $do_sign = $this->late_ch_model->ch_do_sign($ch_id);
            //die(print_r($do_sign));
            if ($do_sign == 0) {
              foreach ($signatures as $ch_signature) {
                $this->late_ch_model->add_signature($ch_id, $ch_signature['role'], $ch_signature['department'], $ch_signature['rank']);
              } 
            }
       //   $this->late_ch_model->add_signature($ch_id, 0, 0);    
          redirect('/late_ch/ch_stage/'.$ch_id);
        }   
      } 
      try {
        ...
        if ($this->input->post('submit')) {
          ...
          $this->data['uploads'] = $this->late_ch_model->getby_fille($this->data['assumed_id']);
        } else {
          $this->data['assumed_id'] = strtoupper(str_pad(dechex( mt_rand( 0, 1048575 ) ), 5, '0', STR_PAD_LEFT));
          $this->data['uploads'] = array();
        }
        $this->load->view('ch_add_new',$this->data);
      }
      catch( Exception $e) {
  ...
  public function make_offer($ch_id) {
    $file_name = $this->do_upload("upload");
    if (!$file_name) {
      die(json_encode($this->data['error']));
    } else {
      $this->load->model("late_ch_model");
      $this->late_ch_model->add($ch_id, $file_name);
      // $this->logger->log_event($this->data['user_id'], "Offer", "projects", $ch_id, json_encode(array("offer_name" => $file_name)), "user uploaded an offer");//log
      die("{}");
    }
  }

  public function remove_offer($ch_id, $id) {
    $file_name = $_POST['key'];
    if (!$id) {
      die(json_encode($this->data['error']));
    } else {
      $this->load->model("late_ch_model");
      $this->late_ch_model->remove($id);
      // $this->logger->log_event($this->data['user_id'], "Offer-Remove", "projects", $ch_id, json_encode(array("offer_id" => $id, "file_name" => $file_name)), "user removed an offer");//log
      die("{}");
    }
  }
  ...
  public function no_ch() {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_sky']) && $this->data['is_sky']) || (isset($this->data['is_UK']) && $this->data['is_UK'])) {
      ...
      try {
        ...
        $this->load->view('ch_no',$this->data);
      }
      catch( Exception $e) {
        show_error($e->getMessage()." _ ". $e->getTraceAsString());
      }
  ...
  public function view($ch_id) {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_sky']) && $this->data['is_sky']) || (isset($this->data['is_UK']) && $this->data['is_UK'])) {
      ...
      $this->data['ch'] = $this->late_ch_model->get_ch($ch_id);
      $this->data['ch_room'] = $this->late_ch_model->get_ch_room($ch_id);
      $this->data['uploads'] = $this->late_ch_model->getby_fille($ch_id);
      $this->data['get_comment'] = $this->late_ch_model->get_comment($ch_id);
      $this->data['signature_path'] = '/assets/uploads/signatures/';
      $this->data['signers'] = $this->get_signers($this->data['ch']['id'], $this->data['ch']['hotel_id']);
      $editor = FALSE;
      $unsign_enable = FALSE;
      $first = TRUE;
      $force_edit = FALSE;
      ...
      if (isset($this->data['user_id'])) {
        ...
        if ( $this->data['ch']['user_id'] == $this->data['user_id'] &&  $this->data['ch']['state_id'] == 1) {
          $editor = TRUE;
        }
      }
      ...
      $this->data['id'] = $ch_id;
      $this->load->view('ch_view', $this->data);
    }
  }
    ...
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_sky']) && $this->data['is_sky']) || (isset($this->data['is_UK']) && $this->data['is_UK'])) {
      ...
      if ($this->input->post('submit')) {
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          $this->late_ch_model->update_ch($form_data, $ch_id);
          foreach ($this->input->post('items') as $item) {
            $item['ch_id'] = $ch_id;  
              //die(print_r($item));    
            $this->late_ch_model->update_room($item, $ch_id, $item['id']);
            }
          redirect('/late_ch/view/'.$ch_id);
        }   
        } 
        try {
        $this->load->helper('form');
        ...
        $this->data['ch_room'] = $this->late_ch_model->get_ch_room($ch_id);
        $this->data['hotels'] = $this->hotels_model->getby_user($this->tank_auth->get_user_id());
        $this->data['uploads'] = $this->late_ch_model->getby_fille($this->data['ch']['id']);
        $this->load->view('ch_edit',$this->data);
      }
      catch( Exception $e) {
        show_error($e->getMessage()." _ ". $e->getTraceAsString());
      }
  ...
  public function Comment($ch_id){
      ...
      $this->load->library('form_validation');
        ...
        $this->late_ch_model->insert_comment($comment_data);
      }
      redirect('/late_ch/view/'.$ch_id);
    }
  }
  ...
  public function index() {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_sky']) && $this->data['is_sky']) || (isset($this->data['is_UK']) && $this->data['is_UK'])) {
      ...
      $this->data['ch'] = $this->late_ch_model->view($hotels);
      foreach ($this->data['ch'] as $ke => $ch) {
        $this->data['ch'][$ke]['items'] = $this->late_ch_model->get_ch_room($this->data['ch'][$ke]['id']);
      }
      foreach ($this->data['ch'] as $key => $dcy) {
        $this->data['ch'][$key]['approvals'] = $this->get_signers($this->data['ch'][$key]['id'], $this->data['ch'][$key]['hotel_id']);
      }
      $this->data['hotels'] = $this->hotels_model->getall();
      $user = $this->users_model->get_user_by_id($this->data['user_id'], TRUE);
      $this->load->view('ch_index', $this->data);
    }
  }
  ...
  public function index_app() {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_sky']) && $this->data['is_sky']) || (isset($this->data['is_UK']) && $this->data['is_UK'])) {
      ...
      $this->data['hotels'] = $this->hotels_model->getall();
      $user = $this->users_model->get_user_by_id($this->data['user_id'], TRUE);
      $this->load->view('ch_index_app', $this->data);
    }
  }
  ...
  public function index_wat() {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_sky']) && $this->data['is_sky']) || (isset($this->data['is_UK']) && $this->data['is_UK'])) {
      ...
      $this->data['hotels'] = $this->hotels_model->getall();
      $user = $this->users_model->get_user_by_id($this->data['user_id'], TRUE);
      $this->load->view('ch_index_wat', $this->data);
    }
  }
  ...
  private function roll_signers($signatures, $hotel_id, $ch_id) {
    ...
    foreach ($signatures as $signature) {
      ...
      if ($signature['user_id']) {
        if ($signature['rank'] == 1){
          $this->late_ch_model->update_state($ch_id, 4);
        }elseif ($signature['rank'] == 2){
          $this->late_ch_model->update_state($ch_id, 5);
        }elseif ($signature['rank'] == 3){
          $this->late_ch_model->update_state($ch_id, 2);
        }
        ...
        if ($signature['reject'] == 1) {
          ...
          $this->late_ch_model->update_state($ch_id, 3);
        } 
  ...
  public function sign($signature_id, $reject = FALSE) {
    $this->load->model('late_ch_model');
    $signature_identity = $this->late_ch_model->get_signature_identity($signature_id);
    $signrs = $this->get_signers($signature_identity['ch_id'], $signature_identity['hotel_id']);
    $this->data['ch'] = $this->late_ch_model->get_ch($signature_identity['ch_id']);
    if (array_key_exists($this->data['user_id'], $signrs[$signature_id]['queue'])) {
      if ($reject) {
        $this->late_ch_model->reject($signature_id, $this->data['user_id']);
        redirect('/late_ch/ch_stage/'.$this->data['ch']['id']);  
      } else {
        $this->late_ch_model->sign($signature_id, $this->data['user_id']);
        redirect('/late_ch/ch_stage/'.$signature_identity['ch_id']);  

      }
    }
    redirect('/late_ch/view/'.$signature_identity['ch_id']);
  }
  ...
  public function unsign($signature_id) {
    ...
    $this->load->model('users_model');
    $signature_identity = $this->late_ch_model->get_signature_identity($signature_id);
    $this->late_ch_model->unsign($signature_id);
    $ch = $this->late_ch_model->get_ch($signature_identity['ch_id']);
    redirect('/late_ch/view/'.$signature_identity['ch_id']);
  }

  public function ch_stage($ch_id) {
    $this->load->model('late_ch_model');
    $this->data['ch'] = $this->late_ch_model->get_ch($ch_id);
    if ($this->data['ch']['state_id'] == 0) {
      $this->self_sign($ch_id);
      $this->late_ch_model->update_state($ch_id, 1);
        redirect('/late_ch/ch_stage/'.$ch_id);
    } elseif ($this->data['ch']['state_id'] != 0 && $this->data['ch']['state_id'] != 2 && $this->data['ch']['state_id'] != 3) {
      $queue = $this->notify_signers($ch_id, $this->data['ch']['hotel_id']);
    }elseif ($this->data['ch']['state_id'] == 2){
      $user = $this->users_model->get_user_by_id($this->data['ch']['user_id'], TRUE);
      $queue = $this->approvel_mail($user->fullname, $user->email, $ch_id);
    }elseif ($this->data['ch']['state_id'] == 3){
      $user = $this->users_model->get_user_by_id($this->data['ch']['user_id'], TRUE);
  ...
  private function self_sign($ch_id) {
    $this->load->model('late_ch_model');
    $this->late_ch_model->self_sign($ch_id, $this->data['user_id']);
  }

  private function notify_signers($ch_id) {
    $notified = FALSE;
    $signers = $this->get_signers($ch_id, $this->data['ch']['hotel_id']);
    foreach ($signers as $signer) {
      if (isset($signer['queue'])) {
        $notified = TRUE;
          foreach ($signer['queue'] as $uid => $user) {
            $this->signatures_mail($signer['role'], $user['name'], $user['mail'], $ch_id);
        }
        break;
      }
    }
  ...
  private function signatures_mail($role, $name, $mail, $ch_id) {
    $this->load->library('email');
    $this->load->helper('url');
    $ch_url = base_url().'late_ch/view/'.$ch_id;
    $this->email->from('e-signature@sunrise-resorts.com');
    $this->email->to($mail);
    $this->email->subject("Late Check Out Report No. #{$ch_id}");
    $this->email->message("Dear {$name},<br/>
              <br/>
              Late Check Out Report No. #{$ch_id} requires your signature, Please use the link below:<br/>
  ...
  private function reject_mail($name, $email, $ch_id) {
    ...
    $this->email->message("Dear {$name},<br/>
              <br/>
              Late Check Out Report No. #{{$ch_id} has been rejected, Please use the link below:<br/>
              <a href='{$ch_url}' target='_blank'>{$ch_url}</a><br/>
              "); 
    $mail_result = $this->email->send();
  }
  ...
  private function approvel_mail($name, $email, $ch_id) {
    $this->load->library('email');
    $this->load->helper('url');
    $dcy_url = base_url().'late_ch/view/'.$ch_id;
    $this->email->from('e-signature@sunrise-resorts.com');
    $this->email->to($email);
    $this->email->subject("Late Check Out Report No. #{$ch_id}");
    $this->email->message("Dear {$name},<br/>
              <br/>
              Late Check Out Report No. #{$ch_id} has been approved, Please use the link below:<br/>
              <a href='{$ch_url}' target='_blank'>{$ch_url}</a><br/>
              "); 
    $mail_result = $this->email->send();
  }
  ...
  public function mailto($ch_id) {
    if ($this->input->post('submit')) {
      ...
      if ($this->form_validation->run() == TRUE) {
        ...
        $this->email->from('e-signature@sunrise-resorts.com');
        $this->email->to($email);
        $this->email->subject("A message from {$user->fullname}, Late Check Out Report No. #{$ch_id}");
        $this->email->message("{$user->fullname} sent you a private message regarding Late Check Out Report No. #{$ch_id}:<br/>
                  {$message}<br />
                  <br />
                  Please use the link below to view the Late Check Out Report:
                  <a href='{$ch_url}' target='_blank'>{$ch_url}</a><br/>
                "); 
        $mail_result = $this->email->send();
      }
