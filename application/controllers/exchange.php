...
class exchange extends CI_Controller {
  ...
  public function __construct() {
    ...
    $this->data['menu']['active'] = "exchange";
  }

  public function add() {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_admin']) && $this->data['is_admin']) || (isset($this->data['is_rater']) && $this->data['is_rater']) || (isset($this->data['is_cluster']) && $this->data['is_cluster']) || (isset($this->data['is_fc']) && $this->data['is_fc'])) {
      if ($this->input->post('submit')) {
        ...
        if ($this->form_validation->run() == TRUE) {
          ...
          $ex_id = $this->exchange_model->create_exchange($datas);
          if (!$ex_id) {
              die("ERROR");
          }
          redirect('/exchange/submit/'.$ex_id);
      ...
      try {
        ...
        $this->load->view('exchange_add',$this->data);
      }
      catch( Exception $e) {
        ...
        if ($this->form_validation->run() == TRUE) {
          ...
          $this->load->model('users_model');  
             ...
             foreach ($this->input->post('banks') as $bank) {
              ...
              $rate_id = $this->exchange_model->create_bank_rate($bank);
          ...
          // $this->exchange_model->update_bank($data);
          $this->data['exchange'] = $this->exchange_model->get_exchange($ex_id);
          $signatures = $this->exchange_model->ex_sign();
          $do_sign = $this->exchange_model->ex_do_sign($ex_id);
            //die(print_r($do_sign));
            if ($do_sign == 0) {
              foreach ($signatures as $ex_signature) {
                $this->exchange_model->add_signature($ex_id, $ex_signature['role'], $ex_signature['department'], $ex_signature['rank']);
              }
            }
          redirect('/exchange/exchange_stage/'.$ex_id);
        }   
      }
      try {
        $this->load->helper('form');
        ...
        $this->data['exchange'] = $this->exchange_model->get_exchange($ex_id);
        $this->data['hotels'] = $this->hotels_model->getby_user($this->tank_auth->get_user_id());
        $this->data['banks'] = $this->exchange_model->getall_banks();
        $this->load->view('exchange_add_new',$this->data);
      }
      catch( Exception $e) {
        show_error($e->getMessage()." _ ". $e->getTraceAsString());
      }
  ...
  public function view($ex_id) {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_admin']) && $this->data['is_admin']) || (isset($this->data['is_rater']) && $this->data['is_rater']) || (isset($this->data['is_cluster']) && $this->data['is_cluster']) || (isset($this->data['is_fc']) && $this->data['is_fc'])) {
      ...
      $this->data['exchange'] = $this->exchange_model->get_exchange($ex_id);
      $this->data['bankss'] = $this->exchange_model->get_bank_rate($ex_id);
      $this->data['GetComment'] = $this->exchange_model->GetComment($ex_id);
      ...
      $bankss = $this->exchange_model->get_bank_rate($ex_id);
      ...
      $this->load->view('exchange_view', $this->data);
    }else{
        redirect('/unknown');
    }
  }
  ...
  public function view_rate($ex_id) {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_admin']) && $this->data['is_admin']) || (isset($this->data['is_rater']) && $this->data['is_rater']) || (isset($this->data['is_cluster']) && $this->data['is_cluster']) || (isset($this->data['is_fc']) && $this->data['is_fc'])) {
      ...
      $this->data['is_editor'] = ( (($this->data['unsign_enable'] || $editor)) || ($force_edit) )? TRUE : FALSE;
      $user = $this->users_model->get_user_by_id($this->data['user_id'], TRUE);
      $this->load->view('exchange_view_rate', $this->data);
    }else{
        redirect('/unknown');
    }
  }
      ...
      if ($this->input->post('submit')) {
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          foreach ($this->input->post('bankss') as $banks) {
            ...
            $this->exchange_model->update_bank_rate($banks, $ex_id, $banks['id']);
          } 
         //die(print_r($this->input->post('bankss')));
          redirect('/exchange/view/'.$ex_id);
      }
    }
      try {
        $this->load->helper('form');
        ...
        $this->data['hotels'] = $this->hotels_model->getby_user($this->tank_auth->get_user_id());
        $this->data['banks'] = $this->exchange_model->getall_banks();
        $this->load->view('exchange_edit',$this->data);
      }
      catch( Exception $e) {
        show_error($e->getMessage()." _ ". $e->getTraceAsString());
      }
    ...
    foreach ($signatures as $signature) {
      ...
      if ($signature['user_id']) {
        if ($signature['rank'] == 1 && $exchange['state_id'] == 1){
          $this->exchange_model->update_state($ex_id, 4);
        }elseif ($signature['rank'] == 2 && $exchange['state_id'] == 4){
          $this->exchange_model->update_state($ex_id, 5);
        }elseif ($signature['rank'] == 3 && $exchange['state_id'] == 5){
          $this->exchange_model->update_state($ex_id, 6);
        }elseif ($signature['rank'] == 4 && $exchange['state_id'] == 6){
          $this->exchange_model->update_state($ex_id, 2);
        }
        ...
        if ($signature['reject'] == 1) {
          ...
          $this->exchange_model->update_state($ex_id, 3);
        } 
  ...
  public function index() {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_admin']) && $this->data['is_admin']) || (isset($this->data['is_rater']) && $this->data['is_rater']) || (isset($this->data['is_cluster']) && $this->data['is_cluster']) || (isset($this->data['is_fc']) && $this->data['is_fc'])) {
      ...
      $this->data['exchange'] = $this->exchange_model->view($hotels);
      foreach ($this->data['exchange'] as $bnk => $ex) {
        $this->data['exchange'][$bnk]['bankss'] = $this->exchange_model->get_bank_rate($this->data['exchange'][$bnk]['id']);
      }
      ...
      $this->data['hotels'] = $this->hotels_model->getall();
      $user = $this->users_model->get_user_by_id($this->data['user_id'], TRUE);
      $this->load->view('exchange_index', $this->data);
    }else{
        redirect('/unknown');
    }
  }
  ...
  public function index_app() {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_admin']) && $this->data['is_admin']) || (isset($this->data['is_rater']) && $this->data['is_rater']) || (isset($this->data['is_cluster']) && $this->data['is_cluster']) || (isset($this->data['is_fc']) && $this->data['is_fc'])) {
      ...
      $this->data['hotels'] = $this->hotels_model->getall();
      $user = $this->users_model->get_user_by_id($this->data['user_id'], TRUE);
      $this->load->view('exchange_index_app', $this->data);
    }else{
        redirect('/unknown');
    }
  }
  ...
  public function index_wat() {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_admin']) && $this->data['is_admin']) || (isset($this->data['is_rater']) && $this->data['is_rater']) || (isset($this->data['is_cluster']) && $this->data['is_cluster']) || (isset($this->data['is_fc']) && $this->data['is_fc'])) {
      ...
      $this->data['hotels'] = $this->hotels_model->getall();
      $user = $this->users_model->get_user_by_id($this->data['user_id'], TRUE);
      $this->load->view('exchange_index_wat', $this->data);
    }else{
        redirect('/unknown');
    }
  }
  ...
  public function index_rej() {
    if ((isset($this->data['is_cairo']) && $this->data['is_cairo']) || (isset($this->data['is_admin']) && $this->data['is_admin']) || (isset($this->data['is_rater']) && $this->data['is_rater']) || (isset($this->data['is_cluster']) && $this->data['is_cluster']) || (isset($this->data['is_fc']) && $this->data['is_fc'])) {
      ...
      $this->data['hotels'] = $this->hotels_model->getall();
      $user = $this->users_model->get_user_by_id($this->data['user_id'], TRUE);
      $this->load->view('exchange_index_rej', $this->data);
    }else{
        redirect('/unknown');
    }
  }
  ...
  public function sign($signature_id, $reject = FALSE) {
    $this->load->model('exchange_model');
    $signature_identity = $this->exchange_model->get_signature_identity($signature_id);
    $signrs = $this->get_signers($signature_identity['ex_id'], $signature_identity['hotel_id']);
    $this->data['exchange'] = $this->exchange_model->get_exchange($signature_identity['ex_id']);
    if (array_key_exists($this->data['user_id'], $signrs[$signature_id]['queue'])) {
      if ($reject) {
        $this->exchange_model->reject($signature_id, $this->data['user_id']);
        redirect('/exchange/exchange_stage/'.$this->data['exchange']['id']);  
      } else {
        $this->exchange_model->sign($signature_id, $this->data['user_id']);
        redirect('/exchange/exchange_stage/'.$signature_identity['ex_id']);  

      }
    }
    redirect('/exchange/view/'.$signature_identity['ex_id']);
  }
  ...
  public function exchange_stage($ex_id) {
    ...
    if ($this->data['exchange']['state_id'] == 0) {
      ...
      $this->exchange_model->update_state($ex_id, 1);
        redirect('/exchange/exchange_stage/'.$ex_id);
      ...
      $raters = $this->exchange_model->getall_raters($ex_id);
  ...
  private function signatures_mail($role, $department, $name, $mail, $ex_id) {
    ...
    $ex_url = base_url().'exchange/view/'.$ex_id;
  ...
  public function unsign($signature_id) {
    ...
    $this->load->model('users_model');
    $signature_identity = $this->exchange_model->get_signature_identity($signature_id);
    $this->exchange_model->unsign($signature_id);
    $exchange = $this->exchange_model->get_exchange($signature_identity['ex_id']);
    redirect('/exchange/view/'.$signature_identity['ex_id']);
  }
  ...
  public function mailme($ex_id) {
   ...
   private function self_sign($ex_id) {
    $this->load->model('exchange_model');
    $this->exchange_model->self_sign($ex_id, $this->data['user_id']);
  }
  ...
  public function Comment($ex_id){
      ...
      $this->form_validation->set_rules('comment','Comment','trim|required');
        ...
        $this->exchange_model->InsertComment($comment_data);
